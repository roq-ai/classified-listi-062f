...
import { getListings } from 'apiSdk/listings';
import { ListingInterface } from 'interfaces/listing';
import { getUserRoles } from 'apiSdk/user-roles';
...
function DashboardPage() {
  ...
  const listingStat = useSWR<PaginatedInterface<ListingInterface>>(
    () => `/listings`,
    () => getListings({ limit: 1, offset: 0 }),
  );

  const userRoleStat = useSWR<PaginatedInterface<UserRoleInterface>>(
    () => `/user-roles`,
    () => getUserRoles({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Listings',
      result: listingStat?.data?.totalCount,
      isLoading: listingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'User Roles',
      result: userRoleStat?.data?.totalCount,
      isLoading: userRoleStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
