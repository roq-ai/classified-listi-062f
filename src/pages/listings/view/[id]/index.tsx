...
import { getListingById } from 'apiSdk/listings';
import { ListingInterface } from 'interfaces/listing';

function ListingViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ListingInterface>(
    () => (id ? `/listings/${id}` : null),
    () =>
      getListingById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('listing', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/listings/edit/${id}`} passHref legacyBehavior>
                    <Button
